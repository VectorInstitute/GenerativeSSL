...
def save_images(all_outs, num_samples, size):
    ...
    all_images_mosaic = np.zeros((3, size*(int(np.sqrt(num_samples))), size*(int(np.sqrt(num_samples)))))
...
def main():
    ...
    input_image_tensor = icgan_inference.preprocess_input_image(input_image_path="../rcdm/images/Jules.jpeg", size=256)

    # Run inference
    generated_images = icgan_inference.run_inference(input_image_tensor=input_image_tensor)
