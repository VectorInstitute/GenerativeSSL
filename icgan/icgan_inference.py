...
class ICGANInference:
    ...
    def run_inference(self, input_image_tensor):
        ...
        noise_vector = truncnorm.rvs(-2*self.config.truncation, 2*self.config.truncation, size=(self.config.num_samples, self.config.noise_size)).astype(np.float32)
        noise_vector = torch.tensor(noise_vector, requires_grad=False, device='cuda')
        instance_vector = torch.tensor(input_features, requires_grad=False, device='cuda').repeat(self.config.num_samples, 1)
        input_label = None
        ...
        for i_bs in range(self.config.num_samples//self.config.batch_size+1):
            ...
            if start == end:
                break
            out = self.get_output(noise_vector[start:end], input_label[start:end] if input_label is not None else None, instance_vector[start:end] if instance_vector is not None else None)
